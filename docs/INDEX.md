# 📚 Índice de Documentación - ACA 4.0

**Última actualización:** 2025-11-13

---

## 🎯 INICIO RÁPIDO (LEE PRIMERO)

### **📌 Documento CORE (Raíz del proyecto):**

**[../CONTEXTO_PROYECTO.md](../CONTEXTO_PROYECTO.md)** 🎯 **PEGAR AL INICIO DE CADA SESIÓN**
- Resumen ejecutivo del proyecto
- Puntos críticos resumidos
- Estructura y flujos principales
- Estado actual y cambios recientes

**[../ESTRUCTURA_PROYECTO.md](../ESTRUCTURA_PROYECTO.md)** 📁
- Árbol completo de directorios
- Dónde está cada cosa
- Patrones de código comunes
- Navegación rápida

### **Documentos esenciales (Carpeta docs/):**

1. **[NO_OLVIDAR.md](NO_OLVIDAR.md)** ⚠️ **LEER SIEMPRE PRIMERO**
   - Puntos críticos del sistema
   - Errores comunes a evitar
   - Checklist antes de modificar código

2. **[ESTADO_ACTUAL_SISTEMA.md](ESTADO_ACTUAL_SISTEMA.md)** 📊
   - Estado completo y actualizado del sistema
   - Estructura de BD
   - Flujos de subida/descarga
   - Configuración crítica

3. **[REFERENCIA_RAPIDA.md](REFERENCIA_RAPIDA.md)** ⚡
   - Comandos frecuentes
   - Consultas útiles
   - Troubleshooting rápido

4. **[INDEX.md](INDEX.md)** 📚
   - Este archivo - Índice completo de documentación

---

## 📝 CAMBIOS Y ACTUALIZACIONES

4. **[CAMBIOS_2025-11-13.md](CAMBIOS_2025-11-13.md)** 
   - Log detallado de cambios de hoy
   - 14 problemas resueltos
   - Estadísticas de la sesión

5. **[RESUMEN_CORRECCIONES_CODIGO.md](RESUMEN_CORRECCIONES_CODIGO.md)**
   - Correcciones aplicadas
   - Código antes/después
   - Verificaciones realizadas

6. **[CAMBIOS_RECIENTES.md](CAMBIOS_RECIENTES.md)**
   - Historial de cambios
   - Evolución del sistema

---

## 🔍 ANÁLISIS Y COMPARACIONES

7. **[COMPARACION_MENU_INFORMACION.md](COMPARACION_MENU_INFORMACION.md)**
   - Comparación documentación vs código implementado
   - Discrepancias encontradas
   - Correcciones necesarias

8. **[CORRECCION_MENU_INFORMACION.md](CORRECCION_MENU_INFORMACION.md)**
   - Problema: menú atrapado
   - Solución: enrutamiento de callbacks
   - Flujo corregido

9. **[ANALISIS_SQL_MIGRACION.md](ANALISIS_SQL_MIGRACION.md)**
   - Análisis de migraciones SQL
   - Campos agregados/renombrados
   - Validación de estructura

---

## 🗄️ BASE DE DATOS

10. **[ESTRUCTURA_REAL_SUPABASE.md](ESTRUCTURA_REAL_SUPABASE.md)**
    - Verificación de estructura actual
    - Tablas y campos existentes
    - Estado de migraciones

11. **[REPORTE_ALINEACION_BD.md](REPORTE_ALINEACION_BD.md)**
    - Alineación código-BD
    - Campos verificados
    - Índices creados

---

## 📂 GESTIÓN DE ARCHIVOS

12. **[LOGICA_DESCARGA_EMPRESA.md](LOGICA_DESCARGA_EMPRESA.md)** ⭐
    - Flujo completo de descarga
    - Diagrama: usuario con 1 vs múltiples empresas
    - Verificación cuándo preguntar empresa

13. **[PROCESO_GESTION_ARCHIVOS.md](PROCESO_GESTION_ARCHIVOS.md)**
    - Definición original de procesos
    - Estados del proceso
    - Datos que guarda la sesión

14. **[PLAN_IMPLEMENTACION_ARCHIVOS.md](PLAN_IMPLEMENTACION_ARCHIVOS.md)**
    - Plan original de implementación
    - Cambios requeridos en BD
    - Componentes necesarios

15. **[RESUMEN_IMPLEMENTACION_ARCHIVOS.md](RESUMEN_IMPLEMENTACION_ARCHIVOS.md)**
    - Resumen de implementación
    - Flujos implementados
    - Dependencias agregadas

---

## 🔐 ROLES Y SEGURIDAD

16. **[SISTEMA_ROLES_PERMISOS.md](SISTEMA_ROLES_PERMISOS.md)**
    - Definición de roles
    - Permisos por rol
    - Matriz de permisos

17. **[EXPLICACION_MULTIEMPRESA_SEGURIDAD.md](EXPLICACION_MULTIEMPRESA_SEGURIDAD.md)**
    - Concepto multiempresa
    - Validaciones de seguridad
    - Ejemplos de código

18. **[EXPLICACION_ACTUALIZAR_HANDLERS.md](EXPLICACION_ACTUALIZAR_HANDLERS.md)**
    - Cómo actualizar handlers
    - Validaciones de permisos
    - Ejemplos antes/después

---

## 🧠 CONCEPTOS E INTEGRACIÓN

19. **[CONCEPTO_HISTORIAL_OPENAI.md](CONCEPTO_HISTORIAL_OPENAI.md)**
    - Historial conversacional
    - Integración con OpenAI
    - Extracción de intención con IA

---

## 📋 PLANIFICACIÓN Y SEGUIMIENTO

20. **[PENDIENTES.md](PENDIENTES.md)**
    - Tareas pendientes
    - Prioridades
    - Mejoras futuras

21. **[PLAN_REVISADO_ARCHIVOS.md](PLAN_REVISADO_ARCHIVOS.md)**
    - Plan revisado de archivos
    - Campos esenciales
    - Estructura final

---

## 📖 RESÚMENES GENERALES

22. **[RESUMEN_SISTEMA_COMPLETO.md](RESUMEN_SISTEMA_COMPLETO.md)**
    - Resumen general del sistema
    - Arquitectura completa
    - Funcionalidades

23. **[RESUMEN_ACTUALIZACIONES_MULTIEMPRESA.md](RESUMEN_ACTUALIZACIONES_MULTIEMPRESA.md)**
    - Actualizaciones para multiempresa
    - Archivos a modificar
    - Validaciones necesarias

24. **[RESUMEN_CAMBIOS_ARCHIVOS.md](RESUMEN_CAMBIOS_ARCHIVOS.md)**
    - Cambios en sistema de archivos
    - Simplificaciones aplicadas
    - Mejoras implementadas

25. **[RESUMEN_SESION_CREACION.md](RESUMEN_SESION_CREACION.md)**
    - Sesión de creación original
    - Decisiones tomadas
    - Contexto inicial

26. **[RESUMEN_REVISION_TESTING.md](RESUMEN_REVISION_TESTING.md)**
    - Revisión y testing
    - Usuarios de prueba
    - Verificaciones realizadas

---

## 🔧 ARCHIVOS TÉCNICOS

27. **[ARCHIVOS_ACTUALIZAR_MULTIEMPRESA.md](ARCHIVOS_ACTUALIZAR_MULTIEMPRESA.md)**
    - Lista de archivos a actualizar
    - Validaciones por archivo
    - Prioridades

---

## 📌 CÓMO USAR ESTA DOCUMENTACIÓN

### **Para hacer cambios al sistema:**
1. Lee **NO_OLVIDAR.md** ← **OBLIGATORIO**
2. Lee **ESTADO_ACTUAL_SISTEMA.md** para entender estado actual
3. Consulta documento específico según lo que vayas a modificar
4. Actualiza documentación después de hacer cambios

### **Para debugging:**
1. **REFERENCIA_RAPIDA.md** → comandos útiles
2. **CAMBIOS_2025-11-13.md** → problemas ya resueltos
3. Logs del sistema con grep de emojis

### **Para nuevos desarrolladores:**
1. **ESTADO_ACTUAL_SISTEMA.md** → Vista general
2. **RESUMEN_SISTEMA_COMPLETO.md** → Arquitectura
3. **REFERENCIA_RAPIDA.md** → Guía práctica

---

## ⚠️ DOCUMENTOS DEPRECADOS

**NO usar estos documentos (pueden contener información desactualizada):**
- Cualquier documento sin fecha de actualización reciente
- Documentos con estado "⚠️ DISCREPANCIAS ENCONTRADAS"
- Planes que ya fueron implementados

**Siempre verificar fecha de "Última actualización" en cada documento.**

---

## 🔄 MANTENIMIENTO DE DOCUMENTACIÓN

### **Cuándo actualizar:**
- ✅ Después de resolver un problema crítico
- ✅ Después de cambiar flujos principales
- ✅ Después de modificar estructura de BD
- ✅ Después de agregar/modificar comandos

### **Qué actualizar:**
1. Documento específico del área modificada
2. **../CONTEXTO_PROYECTO.md** si cambia el estado general
3. **NO_OLVIDAR.md** si es un punto crítico
4. **ESTADO_ACTUAL_SISTEMA.md** si cambia comportamiento general
5. Este **INDEX.md** si se agrega/elimina documentación

---

## 🧪 SCRIPTS DE TESTING

**Ubicación:** `../scripts_testing/`  
**Documentación:** [../scripts_testing/README.md](../scripts_testing/README.md)

### **Scripts de verificación (solo lectura):**
- `verificar_sistema_completo.py` - Verificación end-to-end
- `revisar_estructura_supabase.py` - Estructura de BD
- `verificar_archivos.py` - Archivos en Storage
- `verificar_sesiones.py` - Sesiones activas
- `verificar_bd.py` - Estado de BD

### **Scripts de administración (modifican datos):**
- `asignar_roles_usuarios.py` - Asignar roles a usuarios
- `asociar_empresa_usuario.py` - Asociar usuario-empresa
- `crear_empresa_factorit.py` - Crear empresa Factor IT
- `ejecutar_migracion_roles.py` - Ejecutar migraciones

⚠️ **Scripts de administración modifican la base de datos** - Revisar código antes de ejecutar

---

## 📁 ESTRUCTURA DE CARPETAS

```
aca_4/
├── CONTEXTO_PROYECTO.md          ← 🎯 Documento CORE
├── ESTRUCTURA_PROYECTO.md         ← Mapa del proyecto
├── app/                           ← Código de la aplicación
├── database/migrations/           ← Migraciones SQL
├── docs/                          ← Esta carpeta (documentación)
├── scripts_testing/               ← Scripts de testing
└── venv/                          ← Entorno virtual
```

---

**Última actualización:** 2025-11-13

